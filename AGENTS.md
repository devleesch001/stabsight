...
4. **Une tâche = une unité testée.** Ne marque pas une tâche de `tasks.md` comme terminée sans les tests correspondants qui passent avec `-race`. Ne fais pas de commit "gros paquet" qui mélange plusieurs tâches non liées.
