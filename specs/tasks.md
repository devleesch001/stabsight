...
- [ ] **Task 1.1 :** Créer la structure des répertoires (ex: `cmd/`, `internal/config/`, `internal/scheduler/`, `internal/probes/`).
- [ ] **Task 1.2 :** Mettre en place la commande racine `cobra` (`internet-monitor run` a minima) avec les flags globaux (`--config`, `--log-level`, `--otlp-endpoint`, `--metrics-addr`).
...
- [ ] **Task 3.2 :** Instancier l'exporter Prometheus pour exposer le endpoint `/metrics`.
- [ ] **Task 3.3 :** Instancier l'exporter OTLP (gRPC/HTTP) basé sur les variables d'environnement (standard OTel).
